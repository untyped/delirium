...
Delirium.api.openAndWait = function (kUrl, openUrl) {
  ...
  Delirium.target.contentWindow.location.href = openUrl;
};
...
Delirium.api.reloadAndWait = function (kUrl) {
  ...
  Delirium.registerDefaultWaitHook(kUrl);
  Delirium.target.contentWindow.location.reload();
};
...
Delirium.api.backAndWait = function (kUrl) {
  ...
  try {  
    ...
    Delirium.target.contentWindow.location.href = url;
  } catch (exn) {
    ...
    Delirium.forEach(function (node) { 
      ...
      if (node && node.fireEvent) {
        ...
        if (node.fireEvent("onclick") && node.href) {
          Delirium.target.contentWindow.location.href = node.href;          
        }
...
Delirium.api.clickAndWait = function(kUrl, nodes) {
  ...
  try {
    if (nodes.length == 1) {
      ...
      throw [ [ "clickAndWait", kUrl, nodes ], "Exactly one node must be selected." ];
    }
  } catch (exn) {
    Delirium.sendExn(kUrl, exn);
  }
...
Delirium.event.createClickEvent = function () {
  var evt = Delirium.target.contentDocument.createEvent("MouseEvents");
  
  // TODO : Fill in some of this stuff more intelligently:
  evt.initMouseEvent("click", true, true, Delirium.target.contentWindow,
...
Delirium.event.createChangeEvent = function () {
  var evt = Delirium.target.contentDocument.createEvent("HTMLEvents");
