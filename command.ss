...
(define (open/wait url+generate)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (openAndWait ,(embed-url (lambda (request) request))
...
(define (reload/wait)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (reloadAndWait ,(embed-url (lambda (request) request))))))))

; -> void
(define (back/wait)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (backAndWait ,(embed-url (lambda (request) request))))))))

; -> void
(define (forward/wait)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (forwardAndWait ,(embed-url (lambda (request) request))))))))
...
(define (click/wait selector)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (clickAndWait ,(embed-url (lambda (request) request))
                                          ,selector))))))
...
(define (select/wait selector value)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (selectAndWait ,(embed-url (lambda (request) request))
                                           ,selector 
...
(define (enter-text/wait selector value)
  (respond/stmt
   (lambda (embed-url)
     (js (!dot Delirium api (enterTextAndWait ,(embed-url (lambda (request) request))
                                              ,selector 
                                              ,value))))))
